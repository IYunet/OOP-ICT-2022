...
[[maybe_unused]] human_player_t::human_player_t(std::istream &input_stream, std::ostream &output_stream)
  ...
  for (int number = 1; number <= field_t::FIELD_SIZE; number++) {
    for (char letter = 'A'; letter <= 'A' + (char)(field_t::FIELD_SIZE - 1); letter++) {
      possible_moves.push_back(std::to_string(number) + letter);
    }
  }
}
...
void human_player_t::on_duplicate_move(int i, int j) {
  output_stream << possible_moves[i * field_t::FIELD_SIZE + j] << " DUPLICATE! Repeat move!" << std::endl;
}
void human_player_t::on_miss(int i, int j) {
  output_stream << possible_moves[i * field_t::FIELD_SIZE + j] << " MISS! Enemy's turn!" << std::endl;
}
void human_player_t::on_hit(int i, int j) {
  output_stream << possible_moves[i * field_t::FIELD_SIZE + j] << " HIT! Your next move!" << std::endl;
}
void human_player_t::on_kill(int i, int j) {
  output_stream << possible_moves[i * field_t::FIELD_SIZE + j] << " KILL! Your next move!" << std::endl;
}
