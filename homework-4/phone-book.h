...
class phone_book_t {
private:
  std::set<string> unique_number;
  vector<string> names;
  vector<string> numbers;
  call_t calls;
  size_t size_before{};
  size_t size_after{};
  vector<string> number_history;
  vector<double> duration_history;
  vector<double> all_calls;
  vector<double> total_duration;
public:
  /**
  ...
  static bool sortByVal(const std::pair<string, std::map<string,double>> &a,const std::pair<string, std::map<string,double>> &b);
